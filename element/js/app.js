(() => {
    ...
    function initSliders() {
        if (document.querySelector(".popular__slider.swiper")) new core(".popular__slider.swiper", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 7,
            ...
            loop: true,
            navigation: {
                prevEl: ".popular__button.swiper-button-prev",
                nextEl: ".popular__button.swiper-button-next"
            },
            breakpoints: {
                300: {
                    slidesPerView: 1,
        ...
        if (document.querySelector(".testimonials__slider.swiper")) new core(".testimonials__slider.swiper", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".testimonials__button.swiper-button-prev",
                nextEl: ".testimonials__button.swiper-button-next"
            },
            on: {}
        });
    }
    ...
    let script_link = document.querySelector("#marquee a");
    let marquee = document.querySelector("#marquee");
    let linkWidth = script_link.offsetWidth;
    let marqueeWidth = marquee.offsetWidth;
    let numCopies = Math.ceil(marqueeWidth / linkWidth);
    for (let i = 0; i < numCopies; i++) {
        let copy = script_link.cloneNode(true);
        marquee.appendChild(copy);
    }
    let mainColors = document.getElementsByClassName("item-popular__main-color");
    for (let i = 0; i < mainColors.length; i++) {
        let mainColor = mainColors[i];
        let colorText = mainColor.textContent.trim();
        mainColor.style.backgroundColor = colorText;
        let secondaryColor = mainColor.nextElementSibling;
        if (secondaryColor) if ("" !== secondaryColor.textContent.trim()) secondaryColor.style.borderBottomColor = secondaryColor.textContent.trim(); else secondaryColor.style.display = "none";
    }
    ...
    window.addEventListener("scroll", (function() {
        ...
        if (scrollPosition >= 15) {
            myElement.classList.add("_scrolled");
            myElement.classList.add("header_scrolled");
        } else {
            myElement.classList.remove("_scrolled");
            myElement.classList.remove("header_scrolled");
        }
    }));
    window["FLS"] = true;
    isWebp();
